...
int main(int ac, char ** av)
{
  ...
  Mat im1c = imread(im1_name);
  Mat im2c = imread(im2_name);
  im1 = im1c; im2 = im2c;
  ...
  while ( nThresh > 5 && nFound < 40 ) {
    ...
    nFound = kpts_1.size() < kpts_2.size() ? kpts_1.size() : kpts_2.size();
  ...
  Mat H = findHomography(Mat(mpts_2), Mat(mpts_1), outlier_mask, RANSAC);
  ...
  imshow( outname + "  ----  outliers removed", outimg);
