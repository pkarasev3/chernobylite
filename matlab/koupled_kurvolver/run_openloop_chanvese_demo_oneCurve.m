function [phi1 phi2 img_show U tt xx yy] = run_openloop_chanvese_demo_oneCurve()
...
img      = img - min(img(:)); 
img      = 10*img / max(img(:));
img      = img .* (1 + 0*randn(size(img))*1e-1 );
img0     = img / max(img(:));
img      = img - mean(img(:));
...
phi1    = reinitializeLevelSetFunction(phi1,1,dX,200,3,3,false() );
phi2    = reinitializeLevelSetFunction(phi2,1,dX,200,3,3,false() );
...
save run_openloop_chanvese_demo t_all delta_abs1 delta_abs2 delta_rel1 delta_rel2 ... 
                               phi1 phi2 img img_show U tt xx yy  mu1_in_out mu2_in_out steps
  ...
  function  [phi dt_a mu_i mu_o] = update_phi( Img, phi, Coupling )
    ...
    %phi  =  reinit_SD(phi, 1, 1, dt0/2, 'ENO2', 2);
