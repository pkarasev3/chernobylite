function [phi1 phi2 img_show U U0 tt xx yy] = run_phi_versus_phistar_control_demo2()
...
img_bad  = -1 + 2*(phi_star < 0) .* ( ( (img < 1e-1) - (img == 0) ) > 1e-3  );
img_bad  = medfilt2(img_bad);
img_bad  = reinit_SD(img_bad, 1, 1, 0.7, 'ENO2', 20);
...
while( ( (max([delta_rel1(end),delta_rel2(end)]) > relTol)  || ... 
  ...
  bTriggerInput1 = 0;
  if( (steps > 350) && ( tsum > 0.01 ) )
    bTriggerInput1 = 1;
    tsum           = 0;
    Uxy1 = [128,151]; % Input U(x,y,t)
    U0   = 10;
    Rin  = 10;
    dU   = img_bad .* (img_bad > -1 ); %U0*Heavi( (Rin - sqrt( ((xx-Uxy1(1))).^2 + ((yy-Uxy1(2))).^2 ) )*1e-1 );
    U    = U + dU - (eps_u * U).^3;
    %phi2( 0 < (dU < 0).*(phi1>0)  ) = -1;
    %phi2( 0 < (dU > 0).*(phi1<=0) ) = +1;
  ...
  CouplingSymmetric = 0*(Heavi(phi1*1e2).*Heavi(phi2*1e2));
  C21        = CouplingSymmetric + (U.^2).*(-Heavi(1e1*phi_star)+Heavi(1e1*phi2));
  prev_phi2  = phi2;
...
fprintf('done! saving .... \n');
save run_control_bridge_demo t_all delta_abs1 delta_abs2 delta_rel1 delta_rel2 ... 
  ...
  function displayLevelSets()
    ...
    idxU = find( abs( U ) > 1e-3 );
    imgb( idxU ) = imgb( idxU )*0.5 + 0.5*U(idxU) / max(abs(U(:))+1e-3);
