...
img      = 0.5 * (img1 + img2);
img      = img .* (1+randn(size(img))*0.005);
...
phi2_star = (img > 0.75)*2.0 - 1.0; phi2_star=10*phi2_star;
...
Idx_Active      = 1; % 0 => none are active
numInputs = 0;
...
if bSaveFinal
  ...
  function  [phi dt_a mu_i mu_o] = update_active_phi( Img, phi, ...
                                                     phiInactive, U)
    ...
    dUin     =  abs(U).* (Heavi(5*U) - Heavi(5*phi));
    
    dphi = delta(phi) .* (dphi + dcpl + dUin);
    
    fprintf('mu_i =%2.2f, mu_o =%2.2, G(I)max =%2.2, f(U)max =%2.2\n',...
