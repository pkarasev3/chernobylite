...
while( ( (max([delta_rel1(end),delta_rel2(end)]) > relTol)  || ... 
  ...
  if( tsum > 0.01 )
    ...
    U0   = 10;
    Rin  = 10;
    dU   = U0*Heavi( (Rin - sqrt( ((xx-Uxy1(1))).^2 + ((yy-Uxy1(2))).^2 ) )*1e-1 );
    U    = U + dU - (eps_u * U).^3;
...
save run_control_bridge_demo t_all delta_abs1 delta_abs2 delta_rel1 delta_rel2 ... 
  ...
  function displayLevelSets()
    ...
    imgb( idxU ) = imgb( idxU ) + U(idxU) / max(abs(U(:))+1e-3);
    ...
    imwrite(img_show,['control_bridge_demo_' num2str_fixed_width(steps) '.png']);
