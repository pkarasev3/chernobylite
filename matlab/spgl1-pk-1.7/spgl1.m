...
'iter_skip'  ,      1 , ... % iteration interval in main loop between printf() calls
...
iter_skip           = options.iter_skip;
log_start_main_loop = 0;
...
while 1
    ...
    %------------------------------------------------------------------
    if (logLevel > 0 ) && (logLevel >= 2 || singleTau || printTau || iter == 0 || stat )
       tauFlag = '              '; subFlag = '';
...
if( logLevel > 0 )
  ...
  if( sum(abs(x)) > 0.0 )
    fprintf('spgl1: tau = %f \t \t |x|_1 = %f \t \t len(x) = %d \t \t ||Ax-b||_2 = %f \n', ... 
               tau, info.xNorm1(end), numel(x), info.rNorm2(end) ); 
  end
end
...
function printf(varargin)
  ...
  if( ~log_start_main_loop )
    printf_basic(varargin{:});
  else
    iter_mod = mod(iter,iter_skip);
    if ( logLevel > 0  && ( iter_mod == 1 || iter_mod == 0 ) )
       fprintf(fid,varargin{:});
    end
  end
end % function printf

function printf_basic(varargin)
  if ( logLevel > 0  )
       fprintf(fid,varargin{:});
  end
end
