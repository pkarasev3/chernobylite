...
g         = groups(:);
gidx      = unique(g);
groups    = spalloc(length(gidx),length(g),numel(g));

Npts       = numel(g);
Mpts       = numel(gidx);
g_step     = round(Npts/Mpts);
id_block  = sparse( [ones(1,g_step) , zeros(1, Npts - g_step ) ] );

tic
parfor i=1:length(gidx)
   %groups(i,:) = circshift( id_block,  [0   g_step*(i-1)  ]   );
   groups(i,:) = ( g(:)' == i );
end
tgroups = toc();
disp([ 'creating GROUPS sparse matrix took: ' num2str(tgroups) ' seconds ... solving now! ']);
